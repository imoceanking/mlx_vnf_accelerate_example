...
hairpin_port_unbind(uint16_t port_id)
{
	...
	for (i = 0; i < pair_port_num; i++) {
		rte_eth_link_get_nowait(pair_port_list[i], &eth_link);
		if (RTE_ETH_LINK_DOWN == eth_link.link_status)
			continue;
...
hairpin_port_bind(uint16_t port_id, int direction)
{
	...
	for (i = 0; i < peer_ports_num; i++) {
		rte_eth_link_get_nowait(peer_ports[i], &eth_link);
		if (RTE_ETH_LINK_DOWN == eth_link.link_status)
			continue;
...
hairpin_two_ports_flows_create(void)
{
	...
	queue.index = dev_info.nb_rx_queues - 1; /* rx hairpin queue index. */
